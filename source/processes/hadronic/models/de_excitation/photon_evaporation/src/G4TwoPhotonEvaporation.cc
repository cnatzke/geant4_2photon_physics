...
G4TwoPhotonEvaporation::EmittedFragments(G4Fragment* nucleus)
{
    ...
    if(fVerbose > 1) {
        G4cout << "G4TwoPhotonEvaporation::EmittedFragment: RDM= "
               << fRDM << " done:" << G4endl;
        if(gammas) { G4cout << gammas << G4endl; }
...
G4TwoPhotonEvaporation::GenerateGammas(G4Fragment* nucleus)
{
    ...
    if(eexc <= Tolerance) { return products; }
    ...
    if(!isDiscrete) {
        ...
        if(0 == fIndex || !level) { return products; }
        ...
        if(ltime < 0.0 || (!fRDM && ltime > fMaxLifeTime)) { return products; }
    ...
    if(std::abs(efinal - eexc) <= Tolerance) { return products; }

    G4double eTransTotal = std::abs(efinal - eexc);
    G4double eGamma1 = .700; // MeV Used for testing purposes
    // basic gaussian sharing of energy
    //G4double eGamma1 = G4RandGauss::shoot(eTransTotal/2., .200);
    if (eGamma1 < 0.0) eGamma1 = 0.;
    if (eGamma1 > eTransTotal) eGamma1 = eTransTotal-.001;
    //G4double eGamma1 = eTransTotal * G4UniformRand();
