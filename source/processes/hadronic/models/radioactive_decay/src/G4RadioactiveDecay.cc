...
void G4RadioactiveDecay::AddUserTwoPhotonDataFile(G4int Z, G4int A, G4String filename)
{
    if (Z < 1 || A < 2)
        G4cout << "Z and A not valid!" << G4endl;
...
void G4RadioactiveDecay::SetDecayBias(G4String filename)
{
    ...
    std::ifstream infile(filename, std::ios::in);
    if (!infile)
        G4Exception("G4RadioactiveDecay::SetDecayBias()", "HAD_RDM_003",
    ...
    for (i = 1; i <= NDecayBin; i++)
        DProfile[i] += DProfile[i - 1];
    for (i = 0; i <= NDecayBin; i++)
        DProfile[i] /= DProfile[NDecayBin];
    ...
    else
    {
        ...
        else
        {
            ...
            for (G4int n = 0; n < NSplit; n++)
            {
                ...
                if (nbin == 1)
                {
                    weight1 = 1. / DProfile[nbin - 1] * (DBin[nbin] - DBin[nbin - 1]) / NSplit;
                }
                else if (nbin > 1)
                {
                    weight1 = 1. / (DProfile[nbin] - DProfile[nbin - 2]) * (DBin[nbin] - DBin[nbin - 1]) / NSplit;
                }
                ...
                for (i = 0; i < theDecayRateVector.size(); i++)
                {
                    ...
                    for (index = 0; index < numberOfSecondaries; index++)
                    {
                        ...
                        else if (((const G4Ions *)(asecondaryparticle->GetDefinition()))->GetExcitationEnergy() > 0. && weight > 0.)
                        {
                            // Compute the gamma
...
#ifdef G4VERBOSE
    if (GetVerboseLevel() > 1)
        G4cout << "Begin decay collimation " << G4endl;
#endif
